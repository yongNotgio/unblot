          ...
          import('../poems.js').then(({ hasUserSaved, savePoem, unsavePoem }) => {
            (async () => {
              ...
              saveBtn.onclick = async () => {
                ...
                if (isSaved) { await unsavePoem(poem.id, currentUser.id); saveBtn.classList.remove('save-active'); utils.showToast(dom, 'Removed from saved'); }
                else { await savePoem(poem.id, currentUser.id); saveBtn.classList.add('save-active'); utils.showToast(dom, 'Saved to collection'); }
              };
            })();
          });
        } else if (saveBtn) {
