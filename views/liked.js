...
export async function renderLiked(dom) {
  if (!currentUser) {
    dom.app.innerHTML = `<div class="text-center" style="padding:3rem 0;color:var(--text-secondary);"><p style="font-size:1.25rem;">Please log in to see your liked poems.</p></div>`;
    return;
  }
  dom.app.innerHTML = `<div class="text-center text-lg" style="padding: 3rem 0;">Loading liked poems...</div>`;
  utils.showLoading(dom, true);

  try {
    const poems = await fetchLikedPoems(currentUser.id);
    ...
    function buildCard(poem, index) {
      ...
      <article class="poem-card-enhanced animate-fade-in stagger-${(index % 4) + 1}" data-poem-id="${poem.id}">
        ...
        <div class="card-actions">
          <button class="card-action-btn like-btn like-active" data-id="${poem.id}">
    ...
    <div class="w-full" style="max-width: 800px; margin: 0 auto; padding: 0 1rem;">
      <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem;">
        <span style="font-size: 1.5rem; font-weight: 600;">Liked Poems</span>
      ...
      <div id="poems-feed" class="poems-feed">
        ${poems.length === 0 ? `
          <div class="text-center py-12" style="color: var(--text-secondary);">
            <p style="font-size: 1.25rem;">No liked poems yet.</p>
            <p style="font-size: 0.875rem; margin-top: 0.5rem;">Like poems to see them here!</p>
          </div>` : cardsHtml}
      </div>
    </div>`;
    ...
    dom.app.innerHTML = `<div class="text-center text-red-600" style="padding: 3rem 0;">Failed to load liked poems: ${err.message || err}</div>`;
  } finally {
